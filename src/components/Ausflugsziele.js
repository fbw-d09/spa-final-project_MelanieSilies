...
import AusflugszieleData from '../data/poi.json';
import { Marker, Popup } from 'react-leaflet';
import * as L from "leaflet";
import orangeMarker from '../assets/Marker/ausflug.png';


import { FaWheelchair,FaHorse,FaPersonSwimming,FaFish,FaShower, FaPersonHiking, FaDog,FaLocationArrow,FaRestroom,FaChildren,} from 'react-icons/fa6';
import {IoTicketSharp, IoFitness, IoRestaurantOutline,} from 'react-icons/io5';
import { FcIdea } from "react-icons/fc";
import { TbWorldWww, TbPigMoney,TbKayak, TbBike } from "react-icons/tb";
    ...
    const orangeIcon = L.icon({
      ...
      useEffect(() => {
          const fetchData = async () => {
            try {
              const promises = AusflugszieleData.features.map((feature) => {
  ...
  return (
    <>
      {AusflugszieleData.features.map((feature, index) => {
        ...
        return (
          <Marker key={index} position={[lat, lng]} icon={orangeIcon} >
            <Popup maxHeight={500}>
...
export default AusflugszieleLayer;
